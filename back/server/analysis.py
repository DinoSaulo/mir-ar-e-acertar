...
class Analysis:
    ...
    def analyse(file_path):
        audio_y, audio_sr = Analysis.read_audio(file_path)

        silent_time = Analysis.silent_time(audio_y)
    ...
    @staticmethod
    def read_audio(path):
        return librosa.load(path, offset=0, sr=44100)

    @staticmethod
    def silent_time(y):
        non_silent_interval = librosa.effects.split(
            y, top_db=30, hop_length=44100)

        song = 0

        for snip in non_silent_interval:
            song += snip[1] - snip[0]

        return 100 - ((song/y.size)*100)
